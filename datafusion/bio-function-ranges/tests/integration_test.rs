...
async fn test_coverage_parquet(ctx: SessionContext) -> Result<()> {
    ...
    let fbrain = format!(
        "CREATE EXTERNAL TABLE fbrain STORED AS PARQUET LOCATION '{}'",
        FBRAIN_PATH
    );
    ctx.sql(fbrain.as_str()).await?;

    let exons = format!(
        "CREATE EXTERNAL TABLE exons STORED AS PARQUET LOCATION '{}'",
        EXONS_PATH
    );
    ...
    let expected_table = format!(
        "CREATE EXTERNAL TABLE expected STORED AS PARQUET LOCATION '{}'",
