...
enum PartitionedAnnotationCache {
    // Parquet is the only cache backend. Variation point-lookups, context
...
async fn scan_context_entity(
    ...
    // Read the dict-enabled Parquet context shard. A missing shard (e.g. `motif`,
    // which has no dataset in any chrom) yields an empty context.
...
fn accumulate_boundaries(
    ...
    for batch in batches {
        ...
        let start_col = batch.column(start_idx).as_ref();
        let alts = AltColumnView::from_batch(batch);
        for row in 0..batch.num_rows() {
            let pos = int64_at(start_col, row).ok_or_else(|| {
                DataFusionError::Execution("count pass: null/unsupported start value".into())
            })?;
...
/// Like `spawn_lookup_partition_worker` but executes ALL partitions of `plan` in
/// id order (= contig position order) into one ordered stream, so a grid worker
/// reads its whole chrom-filtered contig (the VCF provider splits a contig into
/// several byte-budget partitions even at target_partitions=1). The annotation
/// layer then selects the worker's rank range. `partition_id` is the logical
/// ordering / shard id.
fn spawn_lookup_full_contig_worker(
    plan: Arc<dyn ExecutionPlan>,
    task_ctx: Arc<TaskContext>,
    partition_id: usize,
    chrom: String,
    ...
    let (sender, receiver) = tokio::sync::mpsc::channel(queue_batches.max(1));
    let join_handle = tokio::spawn(async move {
        let pc = plan.output_partitioning().partition_count().max(1);
        let mut next_batch_id = 0usize;
        for p in 0..pc {
            let mut stream = plan.execute(p, Arc::clone(&task_ctx))?;
            while let Some(batch_result) = stream.next().await {
                let batch = batch_result?;
                let colocated_delta = drain_colocated_sink(&sink)?;
                let batch_id = next_batch_id;
...
async fn prepare_contig_context(
    ...
    let cache_schema = {
        let variation_path = cache.as_parquet().variation_path(&chrom).ok_or_else(|| {
            DataFusionError::Execution(format!(
                "Parquet cache has no variation shard for chrom {chrom}"
            ))
        })?;
...
#[cfg(test)]
mod port_tests {
    //! Ported VEP behavioural tests grafted during the
    //! sync/master-into-dev-test-2026-07-07 merge. These target the private
    //! `buffer_variant_bounds` / `cache_regions_for_coords` /
    //! `collect_buffer_cache_regions` / `cache_region_index` helpers and the
    //! private `TranscriptCacheRegion` struct, all of which survive on
    //! master v0.14.0 with identical signatures.
    use super::*;
