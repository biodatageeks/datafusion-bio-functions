...
struct LookupProfile {
    ...
    /// The parts of `variation_take`, as the lookup reports them; they add up
    /// to it, less the call overhead. They are a breakdown of that stage, not
    /// further stages, so they stay out of `detail_known()` and get a line of
    /// their own.
    take_offsets: Duration,
    take_payload: Duration,
    take_af_rebuild: Duration,
    take_matched: Duration,
}
...
impl LookupProfile {
    ...
    fn detail_lines(&self) -> Vec<String> {
        ...
        vec![
            ...
            format!(
                "[vep-lookup-profile-detail] take offsets={:.3}s payload={:.3}s af_rebuild={:.3}s matched={:.3}s",
                self.take_offsets.as_secs_f64(),
                self.take_payload.as_secs_f64(),
                self.take_af_rebuild.as_secs_f64(),
                self.take_matched.as_secs_f64(),
            ),
        ]
    }
...
impl VariationLookupStream {
    ...
    fn new(
        ...
        if profile_detailed {
            ...
            eprintln!(
                "{}",
                format_variation_lookup_profile_line(&cache_root_label)
            );
        }
    ...
    fn process_batch(&mut self, vcf_batch: &RecordBatch) -> Result<RecordBatch> {
        ...
        if !pending_probes.is_empty() {
            ...
            for (chrom, mut starts) in pending_starts_by_chrom {
                ...
                starts.dedup();
                // Open (or reuse) this contig's shard. The shared cell is
                // scoped to one contig; ensure_parquet_lookup keeps that guard.
                self.ensure_parquet_lookup(&chrom, &cache_columns, coloc_buf.is_some())?;
                ...
                let taken = {
                    ...
                    // Parquet resolution is stateless (footer PageDir).
                ...
                if let Some(t0) = take_started {
                    ...
                    self.profile.take_matched += taken.timing.matched;
                }
...
mod tests {
    ...
    fn lookup_profile_detail_line_formats_probe_decode_match_breakdown() {
        ...
        profile.take_af_rebuild += Duration::from_millis(5);
        profile.take_matched += Duration::from_millis(1);
        ...
        assert_eq!(
            lines[3],
            "[vep-lookup-profile-detail] take offsets=0.003s payload=0.004s af_rebuild=0.005s matched=0.001s"
        );
    }
    ...
    fn variation_lookup_profile_line_identifies_parquet_backend() {
        let line = format_variation_lookup_profile_line("/cache/variation");

        assert!(line.contains("storage=parquet"));
        assert!(line.contains("cache_root=/cache/variation"));
    }
