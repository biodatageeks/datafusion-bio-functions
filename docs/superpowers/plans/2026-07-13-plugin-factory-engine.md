...
`plugin_cache` exists **only on `master`** (14 files). `origin/dev-test` is 14 commits behind and has
**none** of it — nothing in this plan compiles until this lands. This is invisible until you try.

Per branch policy: never commit to `master`; everything lands on `dev-test` via PR.
...
git fetch origin
git ls-tree --name-only origin/dev-test datafusion/bio-function-vep/src/plugin_cache/ | wc -l   # expect 0
git ls-tree --name-only origin/master   datafusion/bio-function-vep/src/plugin_cache/ | wc -l   # expect 14
```

If the first number is already 14, someone synced it — skip to Task 1.

- [ ] **Step 2: Create the sync branch off `dev-test` and merge `master` into it**

```bash
git worktree add ../datafusion-bio-functions-worktrees/sync-dev-test -b chore/sync-dev-test-with-master origin/dev-test
cd ../datafusion-bio-functions-worktrees/sync-dev-test
git merge origin/master
```

Resolve conflicts if any (expect none — `dev-test` is strictly behind).

- [ ] **Step 3: Verify the tree builds and the plugin_cache tests pass**

```bash
cargo test -p datafusion-bio-function-vep --features parquet-cache plugin_cache
```
Expected: all `plugin_cache::*` tests pass.

- [ ] **Step 4: Push and open the PR into `dev-test`**

```bash
git push -u origin chore/sync-dev-test-with-master
gh pr create --base dev-test --title "chore: sync dev-test with master (brings in plugin_cache)" \
  --body "Mechanical merge. dev-test is 14 commits behind master and contains none of the plugin_cache subsystem, so no plugin-factory work can compile on it."
```

**Do not start Task 1 until this is merged.** Every task below assumes a `dev-test` that contains
`datafusion/bio-function-vep/src/plugin_cache/`.
...
cd /Users/wojtek/Documents/vepyr/datafusion-bio-functions
git fetch origin
git worktree add ../datafusion-bio-functions-worktrees/plugin-engine -b feat/plugin-cache-vcf-provider origin/dev-test
