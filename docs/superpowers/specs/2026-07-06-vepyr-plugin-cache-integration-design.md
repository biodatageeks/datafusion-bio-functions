...
2. `[[match_column]]` declares a **template** string with `{name}` placeholders instead of `engine_attr`:
   ...
   ```
3. **Compile once, evaluate cheaply per record.** The template is parsed **once per plugin** at registry-open into a compiled `Vec<Segment>` (`Literal(String)` | `Attr(usize)`), where each placeholder is resolved to a **fixed positional index** into the attribute namespace (no per-record parse, no string-keyed/`HashMap` lookup, no DataFusion on the hot path). Per consequence the engine fills a small fixed-size `[Option<&str>; N]` of namespace values (borrowed from locals it already has — no allocation), and evaluation is a bounded walk over the K segments appending into a **reused scratch buffer** (cleared each record; one amortized allocation). The discriminator value is genuinely per-record (it depends on that line's aa/position), but its cost is a few `push_str`s — no worse than, and structured to match, the `amino_acid_change`/allele `format!`s the hot path already does today.
4. **Gating rule (preserves missense-only behaviour, byte-identical):** if **any** referenced placeholder resolves to `None`, the whole discriminator is `None` → probe miss → empty output. The namespace population encodes the same validity rules the current `amino_acid_change` enforces (`csq.rs:15`): `ref_aa`/`alt_aa` are `Some` only when `Amino_acids` is a clean single-residue `X/Y` pair; `Protein_position` is `Some` only when non-empty and not a range (`-`). So a non-missense consequence → `ref_aa`/`Protein_position` `None` → discriminator `None` → empty, matching the existing gate exactly.
